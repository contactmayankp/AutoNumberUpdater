...
namespace Logging
{
    ...
    {
        /// <summary>
        /// The complete Log file path
        /// </summary>
        protected static string strLogFilePath = string.Empty;

        /// <summary>
        /// Is the Log to be used or not
        /// </summary>
        protected static bool blnLogIsOn = false;

        /// <summary>
        /// The event Log to log errors to
        /// </summary>
        protected static string strEventLogName = string.Empty;
        ...
        /// <summary>
        /// Property - Is the Log to be used
        /// </summary>
        public static bool LogIsOn
        {
            set { blnLogIsOn = value; }
            get { return blnLogIsOn; }
        }

        /// <summary>
        /// Property - Event Log name
        /// </summary>
        public static string EventLogName
        {
            set { strEventLogName = value; }
            get { return strEventLogName; }
        }
        ...
        public static bool ErrorRoutine(bool bLogType, Exception objException)
        {
            try
            {
                ...
                bool bLoggingEnabled = blnLogIsOn;
                ...
                if (true == bLogType)
                {
                    string EventLogName = strEventLogName;
        ...
        /// 
        /// </summary>
        /// <param name="bLogType">The Log Type</param>
        /// <param name="message">The Log Message</param>
        /// <param name="objEventType">The Event Type</param>
        /// <returns></returns>
        public static bool ReportRoutine(bool bLogType, String message, EventLogEntryType objEventType)
        {
            try
            {
                //Check whether logging is enabled or not
                ...
                if (true == bLogType)
                {
                    ...
                    if (!EventLog.SourceExists(EventLogName))
                        EventLog.CreateEventSource(message, EventLogName);

                    // Inserting into event log
                    EventLog Log = new EventLog();
                    Log.Source = EventLogName;
                    Log.WriteEntry(message, objEventType);

                    if (true != CustomErrorRoutine(message))
                        return false;
                }
                    //Custom text-based event log
                else
        ...
        private static bool CustomErrorRoutine(String Message)
        {
            string strPathName = string.Empty;
            if (strLogFilePath.Equals(string.Empty))
            {
            ...
            bool bReturn = true;
            // write the error log to that text file
            if (true != WriteErrorLog(strPathName, Message))
            {
                bReturn = false;
            }
            return bReturn;
        ...
        private static bool WriteErrorLog(string strPathName, String Message)
        {
            bool bReturn = false;
